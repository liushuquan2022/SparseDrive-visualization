...
chn_font = FontProperties(fname=r"fonts/LXGWWenKai-Bold.ttf", size=60)
...
class BEVRender:
    ...
    def render(
        ...
        self._render_legend()
        imgs_path = glob.glob(os.path.join(self.gt_dir, '*.jpg'))
        index = len(imgs_path)
        ...
        self._render_command(data)
        self._render_legend()
        imgs_path = glob.glob(os.path.join(self.pred_dir, '*.jpg'))
        index = len(imgs_path)
...
#        self.axes.imshow(legend, extent=(15, 40, -40, -30))
        self.axes.imshow(legend, extent=(30, 40, -40, -35))

    def _render_command(self, data):
        cmd = data['gt_ego_fut_cmd'].argmax()
        # 绘制控制命令
        # self.axes.text(-38, -38, CMD_LIST[cmd], fontsize=60)
        self.axes.text(-38, -38, CMD_LIST[cmd], font=chn_font)